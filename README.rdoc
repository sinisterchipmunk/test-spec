...
Copyright (C) 2010 Christian Neukirchen <mailto:chneukirchen@gmail.com>
...
You can create a file such as test_helper.rb and configure the global
before and after hooks like so:

  Test::Spec.configure do |config|
    config.before(:each) { . . . }
    config.after(:each) { . . . }
    
    # config.before(:all) and config.after(:all) are not operational
    # at this time.
  end

These fire before/after any test-specific hooks. This means that
config.before(:each) will be executed before a test's before(:all).
 
Simply require the helper file into your tests and you're done.
...
To install from Github:
    git clone git://github.com/sinisterchipmunk/test-spec.git
    cd test-spec
    rake package
    cd pkg
    gem install --local test-spec-0.11.gem

=== Note
The Github installation is currently the only version which includes global
setup/teardown.

=== Installing with Ruby Gems
...
* July 13th, 2010 (Colin MacKenzie IV)
  * Added global before(:each) and after(:each) hooks.
  
* April 20th, 2012 (Colin MacKenzie IV)
  * Fixed errors caused by disabled tests under test-unit 2.4.7.
...
Colin MacKenzie IV:: <http://thoughtsincomputation.com>
